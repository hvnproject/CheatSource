...
bool entity_t::compute_hitbox_surrounding_box(Vector* mins, Vector* maxs)
{
	...
	static auto fn = reinterpret_cast<Fn>(util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F8 81 EC ?? ?? ?? ?? 53 56 8B F1 57 83 BE ?? ?? ?? ?? ??")));
...
void player_t::modify_eye_position(Vector& eye_position)
{
	...
	static auto lookup_bone = reinterpret_cast <int(__thiscall*)(void*, const char*)> (util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 53 56 8B F1 57 83 ? ? ? ? ? ? 75")));
