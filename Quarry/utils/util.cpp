...
namespace util
{
	...
	void UTIL_ClipTraceToPlayers(const Vector& start, const Vector& end, uint32_t mask, CTraceFilter* filter, CGameTrace* tr, float range) 
	{
		static auto func = util::FindSignature(crypt_str("client.dll"), crypt_str("53 8B DC 83 EC 08 83 E4 F0 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 81 EC ? ? ? ? 8B 43 10"));
	...
	float get_interpolation()
	{
		...
		int updaterate = cl_updaterate->GetInt();
		...
		if (sv_minupdaterate && sv_maxupdaterate)
			updaterate = sv_maxupdaterate->GetInt();
