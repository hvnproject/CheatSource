...
void __stdcall hooks::hkFrameStageNotify(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_RENDER_END)
	{
		...
		if (g_cfg.esp.removals[REMOVALS_DECALS])
			...
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					if (current_shot->impact_hit_player)
					{
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && current_shot->shot_info.hitchance > 80)
							eventlogs::get().add(crypt_str("Missed shot due to bad resolve"));
						else if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && current_shot->shot_info.hitchance < 80)
							eventlogs::get().add(crypt_str("Missed shot due to resolver innacuracy"));
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						{
							eventlogs::get().add(crypt_str("Missed shot due to bad hitchance"));
							current_shot->shot_info.result = crypt_str("Bad hitchance");
						}
						else if (current_shot->impacts)
						{
							eventlogs::get().add(crypt_str("Missed shot due to unregistered"));
							current_shot->shot_info.result = crypt_str("Unregistered");
						}
						else if (!current_shot->impacts)
						{
							eventlogs::get().add(crypt_str("Missed shot due to prediction error"));
							current_shot->shot_info.result = crypt_str("Prediction error");
						}
