...
void __fastcall hooks::hkSceneend(void* ecx, void* edx)
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		{
			m_materialsystem()->FindMaterial(crypt_str("Nothing_chams"), nullptr), 
			m_materialsystem()->FindMaterial(crypt_str("Nothing_metallic"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("Nothing_flat"), nullptr),
			...
			m_materialsystem()->FindMaterial(crypt_str("Nothing_glow"), nullptr)
		};
