...
{
	std::string file_path = crypt_str("C:\\Quarry\\Scripts\\CSGO\\");
...
void c_menu::draw_legit(int child)
{
	...
	else
	{
		...
		ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			ImGui::SliderFloat(crypt_str("Silent field of view"), &g_cfg.legitbot.weapon[hooks::legit_weapon].silent_fov, 0.f, 30.f, (!g_cfg.legitbot.weapon[hooks::legit_weapon].silent_fov ? crypt_str("None") : crypt_str("%.2f"))); 
			...
			{
				ImGui::SliderFloat(crypt_str("RCS custom field of view"), &g_cfg.legitbot.weapon[hooks::legit_weapon].custom_rcs_fov, 0.f, 30.f, (!g_cfg.legitbot.weapon[hooks::legit_weapon].custom_rcs_fov ? crypt_str("None") : crypt_str("%.2f"))); 
		...
		if (!child)
		{
			if (!visuals_section)
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					{
						ImGui::Text(crypt_str("Progress color "));
						ImGui::SameLine();
						ImGui::ColorEdit(crypt_str("##progressgrenpredcolor"), &g_cfg.esp.grenade_proximity_warning_progress_color, ALPHA);
					}
		...
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				if (&g_cfg.misc.automatic_peek)
				{
					...
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##autopeekcolor"), &g_cfg.misc.automatic_peek_color, ALPHA);
				}
...
void c_menu::draw_settings(int child)
{
	...
	ImGui::BeginChild(crypt_str("##CONFIGS__FIRST"), ImVec2(686 * dpi_scale, (child_height - 35) * dpi_scale));
	{
		...
		{
			std::string folder = crypt_str("C:\\Quarry\\Configs\\CSGO\\");
...
void c_menu::draw_lua(int child)
{
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			{
				std::string folder = crypt_str("C:\\Quarry\\Scripts\\CSGO\\");
				ShellExecute(NULL, crypt_str("open"), folder.c_str(), NULL, NULL, SW_SHOWNORMAL);
			}
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("Nothing"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(850 * dpi_scale - ImGui::CalcTextSize(crypt_str("Quarry")).x - (11 * dpi_scale), (16 * dpi_scale) - (ImGui::CalcTextSize(crypt_str("Quarry")).y / 2)));

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str("Quarry"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
