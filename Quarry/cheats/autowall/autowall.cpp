...
void autowall::scale_damage(player_t* e, CGameTrace& enterTrace, weapon_info_t* weaponData, float& currentDamage)
{
	...
	//need if server use another values
...
bool autowall::trace_to_exit(CGameTrace& enterTrace, CGameTrace& exitTrace, Vector startPosition, const Vector& direction)
{
	...
	do
	{
		...
		if (enter_point_contents == -1)
			enter_point_contents = point_contents;
		...
		//credits: supremacy
		auto sv_clip_penetration_traces_to_players = m_cvar()->FindVar(crypt_str("sv_clip_penetration_traces_to_players"));

		if (sv_clip_penetration_traces_to_players->GetInt())
			util::UTIL_ClipTraceToPlayers(end, start, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr, &exitTrace, -60.f);
...
bool autowall::handle_bullet_penetration(weapon_info_t* weaponData, CGameTrace& enterTrace, Vector& eyePosition, const Vector& direction, int& possibleHitsRemaining, float& currentDamage, float penetrationPower, float ff_damage_reduction_bullets, float ff_damage_bullet_penetration, bool draw_impact)
{
	...
	if ((possibleHitsRemaining <= 0 && !is_light_surf && !is_solid_surf && enter_material != CHAR_TEX_GRATE && enter_material != CHAR_TEX_GLASS) || penetrationPower <= 0)
		return false;
	...
	if (!exit_surface_data || enter_penetration_modifier <= 0)
		return false;
	...
	float combined_damage_modifier, combined_penetration_modifier;
...
bool autowall::fire_bullet(weapon_t* pWeapon, Vector& direction, bool& visible, float& currentDamage, int& hitbox, IClientEntity* e, float length, const Vector& pos)
{
	...
	while (currentDamage >= 1.0f)
	{
		...
		auto isPlayer = enterTrace.hit_entity && ((player_t*)enterTrace.hit_entity)->is_player();
		auto isEnemy = ((player_t*)enterTrace.hit_entity)->m_iTeamNum() != g_ctx.local()->m_iTeamNum();

		if (util::valid_hitgroup(enterTrace.hitgroup) && isPlayer && isEnemy)
		{
