...
void GrenadePrediction::Paint()
{
	...
	if (target.second && weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE && (!g_cfg.esp.on_click || g_ctx.get_command()->m_buttons & IN_ATTACK || g_ctx.get_command()->m_buttons & IN_ATTACK2)) {
		...
		if (math::world_to_screen(target.second->hitbox_position(HITBOX_HEAD), screen))
			g_render::get().text(g_render::get().esp, screen.x - 4, screen.y - 28, (int)target.first >= target.second->m_iHealth() ? Color::Red : Color::White, true, true, true, false, std::to_string((int)target.first).c_str());
	}
	...
	if (type && path.size() > 1)
	{
		...
		for (auto it = path.begin(); it != path.end(); ++it)
		{
			if (math::world_to_screen(prev, nadeStart) && math::world_to_screen(*it, nadeEnd))
				g_render::get().line((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, g_cfg.esp.grenade_prediction_tracer_color);

			prev = *it;
		}

		if (math::world_to_screen(prev, nadeEnd))
			g_render::get().text(g_render::get().grenades, nadeEnd.x - 8, nadeEnd.y - 5, g_cfg.esp.grenade_prediction_tracer_color, true, true, true, true, weapon->get_icon());

		Vector endpos = path[path.size() - 1];

		if (weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
			g_render::get().Draw3DFilledCircle(endpos, 90, g_cfg.esp.grenade_prediction_tracer_color);
