...
class c_grenade_warning : public singleton< c_grenade_warning >
{
public:
	...
	struct data_t
	{
		...
		__forceinline bool physics_simulate()
		{
			...
			const auto move = Vector(m_velocity.x * m_globals()->m_intervalpertick, m_velocity.y * m_globals()->m_intervalpertick, (m_velocity.z + new_velocity_z) / 2.f * m_globals()->m_intervalpertick);
		...
		{
			TraceHull(src, dst, { -2.f, -2.f, -2.f }, { 2.f, 2.f, 2.f }, mask, m_owner, m_collision_group, &trace);

			if (trace.startsolid && (trace.contents & CONTENTS_CURRENT_90))
			{
				trace.clear();
				TraceHull(src, dst, { -2.f, -2.f, -2.f }, { 2.f, 2.f, 2.f }, mask & ~CONTENTS_CURRENT_90, m_owner, m_collision_group, &trace);
			}

			if (!trace.DidHit() || !trace.hit_entity || !reinterpret_cast<entity_t*>(trace.hit_entity)->is_player())
				return;
		...
		__forceinline void physics_push_entity(const Vector& push, trace_t& trace)
		{
			...
			if (trace.startsolid)
			{
				...
				TraceLine(m_origin - push, m_origin + push, (MASK_SOLID | CONTENTS_CURRENT_90) & ~CONTENTS_MONSTER, m_owner, m_collision_group, &trace);
			}
			...
			if (reinterpret_cast<entity_t*>(trace.hit_entity)->is_player() || m_index != WEAPON_TAGRENADE && m_index != WEAPON_MOLOTOV && m_index != WEAPON_INCGRENADE)
				return;

			static const auto weapon_molotov_maxdetonateslope = m_cvar()->FindVar("weapon_molotov_maxdetonateslope");

			if (m_index != WEAPON_TAGRENADE && trace.plane.normal.z < std::cos(DEG2RAD(weapon_molotov_maxdetonateslope->GetFloat())))
				return;
		...
		__forceinline void perform_fly_collision_resolution(trace_t& trace)
		{
			...
			if (trace.hit_entity)
			{
				if (autowall::get().is_breakable_entity(reinterpret_cast<entity_t*>(trace.hit_entity)))
				{
				BREAKTROUGH:
					m_velocity *= 0.4f;
				...
				{
					if (is_player && m_last_hit_entity == trace.hit_entity)
					{
		...
		__forceinline void predict(const Vector& origin, const Vector& velocity, float throw_time, int offset)
		{
			...
			{
				case WEAPON_SMOKEGRENADE:
					m_next_think_tick = TIME_TO_TICKS(1.5f);
					break;
				case WEAPON_DECOY:
					m_next_think_tick = TIME_TO_TICKS(2.f);
					break;
				case WEAPON_FLASHBANG:
				case WEAPON_HEGRENADE:
