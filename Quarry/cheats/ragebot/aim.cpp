...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position)
{
	...
	{
		if (body_hitboxes && (point.hitbox < HITBOX_NECK || point.hitbox > HITBOX_LEFT_FOREARM))
		{
			body_hitboxes = false;
...
std::vector <scan_point> aim::get_points(adjust_data* record, int hitbox, bool from_aim)
{
	...
	else
	{
		...
		if (hitbox == HITBOX_HEAD)
		{
			...
			points.at(0).point.x -= final_radius;
			points.at(1).point.x += final_radius;
			points.at(2).point.y += final_radius;
			points.at(3).point.z += final_radius;
			points.at(4).point.z -= final_radius;
		}
		else if (hitbox >= HITBOX_PELVIS && hitbox <= HITBOX_UPPER_CHEST)
		{
			...
			points.at(1).point.x += final_radius;
			points.at(2).point.y += final_radius;
			points.at(3).point.y -= final_radius;
			points.at(4).point.z += final_radius;
			points.at(5).point.z -= final_radius;
		}
		else if (hitbox == HITBOX_RIGHT_CALF || hitbox == HITBOX_LEFT_CALF)
		{
			...
			points.at(0).point.y += final_radius;
			points.at(1).point.y -= final_radius;
		}
...
void aim::fire(CUserCmd* cmd)
{
	...
	auto hitchance_amount = !misc::get().double_tap_enabled && !g_ctx.globals.tocharge >= 0 ?
		g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount :
		g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount / 2;

	auto hc_result = hitchance(aim_angle);
