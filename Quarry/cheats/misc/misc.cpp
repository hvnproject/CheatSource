...
#define VERSION crypt_str("Nothing | ")
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("Nothing - stay with us or lose the game."),
		crypt_str("Get good. Get Nothing."),
		crypt_str("Nothing - just the best."),
		crypt_str("Go to another level with Nothing."),
	};
...
void misc::spectators_list()
{
	...
	for (auto i = 0; i < spectators.size(); i++)
	{
		...
		g_render::get().text(g_render::get().logs, width - x, g_cfg.menu.watermark ? y + 30 : y + 6, Color::White, false, false, false, true, spectators.at(i).c_str());
	}
}
