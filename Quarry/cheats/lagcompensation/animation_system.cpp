...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	auto setup_matrix = [&](player_t* e, AnimationLayer* layers, const int& matrix) -> void
	{
		...
		case LOW_SECOND:
			...
			break;
		case LOW_FIRST1:
			e->setup_bones_fixed(record->matrixes_data.low_first1, BONE_USED_BY_ANYTHING);
			break;
		case LOW_SECOND1:
			e->setup_bones_fixed(record->matrixes_data.low_second1, BONE_USED_BY_ANYTHING);
			break;
		}
	...
	if (!player_info.fakeplayer && g_ctx.local()->is_alive() && e->m_iTeamNum() != g_ctx.local()->m_iTeamNum() && ticks_chocked >= 1)
	{
		...
		setup_matrix(e, record->left_layers, LOW_FIRST);
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		...
		setup_matrix(e, record->right_layers, LOW_SECOND);
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		...
		setup_matrix(e, record->left_layers, LOW_FIRST1);
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		...
		setup_matrix(e, record->right_layers, LOW_SECOND1);
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
